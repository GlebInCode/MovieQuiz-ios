...
class MoviesLoaderTests: XCTestCase {
    func testSuccessLoading() throws {
        ...
        // When
        let expectation = expectation(description: "Lading expectation")
        
        loader.loadMovies { result in
            // Then
            switch result {
            case .success(let movies):
                XCTAssertEqual(movies.item.count, 2)
                expextation.fulfill()
            case .failure(_):
    ...
    func testFailureLoading() throws {
        ...
        // When
        let expectation = expectation(description: "Loading ex[extation")
        ...
        wailForExpextations(timeout: 1)
    }
}

struct StubNetworkClient: NetworkRouting { // не знаю где это должно быть размещено
